...
flowchart LR
  ...
  Consumer[consumeFrames_goroutine]
  FrameBus[FrameBus non-blocking fan-out]
  Worker1[Python Worker 1]
  Worker2[Python Worker 2]
  WorkerN[Python Worker N]
  MQTT[MQTT Emitter]

  Stream -- "Frame channel (buffer 10)" --> Consumer
  Consumer -- "frame" --> FrameBus

  FrameBus -. "SendFrame (non-blocking)" .-> Worker1
  FrameBus -. "SendFrame (non-blocking)" .-> Worker2
  FrameBus -. "SendFrame (non-blocking)" .-> WorkerN
